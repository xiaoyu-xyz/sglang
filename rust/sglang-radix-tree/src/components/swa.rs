...
impl SwaComponent {
    ...
    fn attach_swa_host_value_<K: ChildKeyType, V: RadixValue>(
        ...
        node.set_host_value(SWA, host_indices.deep_copy());
...
impl<K: ChildKeyType, V: RadixValue> TreeComponent<K, V> for SwaComponent {
    ...
    fn update_component_on_insert_overlap(
        ...
        if swa_evicted_seqlen <= total_prefix_len {
            ...
            node.set_device_value(FULL, value_slice.deep_copy());
            ...
            node.set_device_value(FULL, new_full.deep_copy());
            cache_actions.push(CacheAction::<V>::FreeDeviceKVFullOnly(vec![old_full]));
            cache_actions.push(CacheAction::<V>::SwaRebuild {
                node_id: node_ext_id,
        ...
        Ok(match phase {
            CacheTransferPhase::BackupHost => {
                ...
                node.try_device_value(SWA).map(|value| {
                    vec![PoolTransfer::<V> {
                        name: PoolName::Swa,
                        device_indices: Some(value.to_i64()),
                        ..Default::default()
                    }]
                })
            }
        ...
        match phase {
            ...
            CacheTransferPhase::LoadBack => {
                ...
                for &loaded_id in transfer.nodes_to_load.iter().flatten() {
                    ...
                    let swa_chunk = device_indices.slice(offset, n_tokens).deep_copy();
